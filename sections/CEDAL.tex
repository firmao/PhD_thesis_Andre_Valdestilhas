...
\begin{algorithm} [htb] 
	...
	\textbf{Output}: An error list with erroneous nodes.
	\begin{algorithmic}[1]
		\Procedure{CEDAL($G(V,E)$)}{}
		\State{$\mathcal{P}$ = getPartitions($G(V,E))$}
		\ForAll{$p \in \mathcal{P}$}~\textbf{(in parallel)}
		\ForAll{$r \in p$}
		push onto $clusterDataset$ r, $MapResourcesDataset.get(r)$
		\EndFor
		\ForAll{$r \in clusterDataset$}
		\If{$countDataset(r) > 1$}
		push onto $ErrorList$ resource.originalFileName + resource.dataset + resource.path
		\EndIf
		\EndFor
		\EndFor
		\State{return $ErrorList$}
		\EndProcedure
		
		\Procedure{getPartitions($G(V,E)$)}{} 
		\ForAll{$v \in V$}
		\State{push onto $MapResourcesDataset$ $v, extracDataset(v)t$}
		\State{push all nodes connected to $v$ onto $\mathcal{V}$} \Comment{UnionFind algorithm}
		\State{push $\mathcal{V}$ onto $\mathcal{P}$}
		\EndFor
